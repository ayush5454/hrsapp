...
namespace dao {
  ...
  SkillCategoryDAO::SkillCategoryDAO()
  {
   ...
   /**@fn create(SkillCategory& SkillCategory)
     ...
     * @param A reference to SkillCategory object containing SkillCategory data
     * @return none
     */       
  ...
  std::vector<HRSObject*> SkillCategoryDAO::find(std::string searchCriteria)
  {
      ...
      if(conn->getError().isError()) //Checks for error.
	{
      ...
      do {
	sklInfo.push_back(new SkillCategory(SkillCategoryAssembler::disAssemble(rs))  ); 
      } while( rs->next() );

//new SkillCategory "class name in common folder"
...
Info - Record Matching the pkValue 
  ...
  HRSObject* SkillCategoryDAO::findByPK(std::string pkValue)
  {
   ...
   /**@fn remove(std::string pkValue)
     * @brief Remove the SkillCategory from the SkillCategory table given the SkillCategoryId
...
// class SkillCategory function info.getCategoryId()
#ifdef ALOGGER
